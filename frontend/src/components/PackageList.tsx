...
const PackageList: React.FC<PackageListProps> = ({ onPackageSelect, refreshTrigger, onNavigateToConversions }) => {
  ...
  const handlePackageAction = async (pkg: NimasPackage, action: string) => {
    switch (action) {
      case 'download':
      ...
      case 'convert':
        try {
          setLoading(true);
          const response = await axios.post(`/api/conversion-jobs/start/${pkg.id}`);
          if (response.status === 200) {
            setMessage({ text: 'Conversion started successfully! Navigating to Conversion Jobs...', type: 'success' });
            // Refresh the package list to show updated status
            loadPackages();
            // Navigate to conversions tab after a short delay to show the success message
            setTimeout(() => {
              if (onNavigateToConversions) {
                onNavigateToConversions();
              }
            }, 1500);
          }
        } catch (error: any) {
          setMessage({ text: error.response?.data || 'Failed to start conversion', type: 'danger' });
        } finally {
          setLoading(false);
        }
        break;
